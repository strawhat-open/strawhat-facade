 ...
 *
 *  Tidings Status 基于http状态码去规划，一来提高共识，二来减少争议，三来可以基于http协议进行规范化
...
public enum TidingsStatus {
    ...
    TidingsStatus(int value, String reasonPhrase) {
        this.value = value;
     ...
     */
    public static TidingsStatus valueOf(int statusCode) {
        TidingsStatus status = resolve(statusCode);
        if (status == null) {
     ...
     * <p>Retrievable via {@link TidingsStatus#series()}.
     */
    public enum Series {
         ...
         */
        public static Series valueOf(TidingsStatus status) {
